...
if has_cmd firefox || [ -d "$HOME/.mozilla/firefox" ]; then
    info "Configuring Firefox (native title bar, dark mode)..."

    if pgrep -x firefox >/dev/null 2>&1; then
        warn "Firefox is running — close it, then re-run this script for changes to apply."
    fi
    ...
    if [ -f "$PROFILES_INI" ]; then
        while IFS= read -r rel_path; do
            ...
            if [[ "$rel_path" = /* ]]; then
                ...
                profile_dir="$HOME/.mozilla/firefox/$rel_path"
            fi
            ...
            ok "Updated Firefox prefs in $(basename "$profile_dir")/user.js"
        done < <(grep -E "^Path=" "$PROFILES_INI" | cut -d= -f2-)
    else
        warn "No Firefox profiles.ini found — skipping Firefox configuration."
    fi
else
